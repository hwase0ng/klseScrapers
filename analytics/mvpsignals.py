    ...
    def evalRetrace(sval):
        ...
        if newhighM and newhighP and newhighV:
            ...
            if prevbottomP:
                ...
                if isprev3bottomM():
                    if isprev3bottomP():
                        if plistP[-1] == min(plistP[-3:]) and nlistP[-1] < 0:
                            # 2013-12-10 PADINI
                            sig, state = -sval8, 1
                elif isprev3topP():
                    if plistM[-1] == min(plistM[-3:]) and plistM[-1] > max(nlistM[-3:]):
    ...
    def evalTopC(sval):
        ...
        if newlowM or lastM == min(nlistM):
            ...
            if topP and not newlowP:
                ...
                if plistP[-1] < 0 and plistV[-1] < 0:  # vvalP == 1:
            ...
            sval0 = sval + 0
            if isprev3topM():
                if tripleP in p3d and tripleP in n3u:
                    if isprev3bottomM():
                        if max(plistM[-3:]) < 10:
                            if nlistP[-1] > 0 and nlistP[-1] > nlistP[-2]:
                                # 2016-12-02 DUFU
    ...
    def evalExtremes(sval):
        ...
        if newhighM and newhighP and newhighV:
            if isprev3bottomM() and isprev3bottomP():
                if tripleM in n3u:
                    if narrowP == 9:
                        # 2018-08-02 DUFU
                        sig, state = sval, 1
        elif tripleM in p3d and tripleP in p3d and tripleV in p3d:
            if newlowC or bottomC:
                ...
                # 2018-03-16 KESM
                sig, state = -sval, 2
        elif vvalP == 2 and (bottomC or prevbottomC):
            ...
            if tripleP in p3u and tripleM in p3d:
                if plistP[-1] > 0 and plistP[-2] < 0:
                    ...
                    # 2014-04-24 CARLSBG
                    sig, state = -sval, 8
        elif bottomC and newhighC and narrowM == 6:
            ...
            if not ssig or sstate > 900:
                if newhighC or (firstC == maxC and lastC > highbar):
                    pass
                        ...
                        (not (topC or prevtopC) and nlistC[-1] > highbar and lastC > min(nlistC)):
                ...
                if newhighC or (firstC == maxC and (lastC > max(plistC) or lastC > highbar)):
                    if prevtopC and nlistC[-1] > highbar:
                        ...
                        if not ssig and not mia:
                            mia = 40
            if not ssig or sstate > 900:
                if topC or prevtopC or (newhighC and plistC[-1] == max(plistC) and
                                        plistC[-1] > midbar) or \
                    ...
                    if not ssig and not mia:
                        mia = 50
            if not ssig or sstate > 900:
                if (bottomC or narrowC == 9) and lastC > highbar:
